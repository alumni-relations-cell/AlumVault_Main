...
import (
	...
	"github.com/your-org/alumni-go/internal/importer"
...
func main() {
	log := logger.Default()
	log.Info().Msg("Starting CSV/sheet import worker...")
	...
	worker := importer.NewWorker(db, ch, cfg)

	log.Info().Msg("Importer started, waiting for jobs...")
	go worker.Start()
