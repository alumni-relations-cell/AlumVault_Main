...
func main() {
	log := logger.Default()
	log.Info().Msg("Starting matching engine worker...")
	...
	worker := matcher.NewWorker(db, ch, cfg)

	log.Info().Msg("Matching engine started, waiting for jobs...")
	go worker.Start()

	quit := make(chan os.Signal, 1)
	signal.Notify(quit, syscall.SIGINT, syscall.SIGTERM)
	<-quit
	log.Info().Msg("Shutting down matcher...")
}
